...
<script>
  // send POST using the same encoding your Python parser expects
  ...
  sliders.forEach((el, i) => {{
    el.addEventListener("input", (e) => {{
      ...
      labels[i].textContent = v;  // update the number live
      sendUpdate(i, v);           // >>> POST on every move (no page reload)
    }});
  }});
</script>
</html>"""
